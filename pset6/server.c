...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // separate line into diffrent variables
            char* method = strtok(line, " ");
            char* request_target = strtok(NULL," ");
            char* http_version = strtok(NULL, "\r\n");

            // error checking as specified by the problem spec
            ...
            // find the length of the request target
            int request_target_length = strlen(request_target) + 1;

            // loop through to error check throughout
            for (int i = 0; i < request_target_length; i++)
            {
            ...
            char* query = request_target;

            if (strstr(query, "name") != NULL)
            {
                query = strstr(query, "name");
            }
            ...
            int root_length = strlen(root) + 1;
            char temp_path[root_length];
            strcpy(temp_path, root);
            char* path = strcat(temp_path, request_target);

            if (strstr(path, "name") != NULL)
            {
                strcpy(temp_path, path);
                path = strtok(temp_path, "?");
            }
            ...
            if (strncasecmp("php", extension, 3) == 0)
            {
                extension = "php";
            }
...
const char* lookup(const char* extension)
{
    ...
    // compare the extension to find out what type
